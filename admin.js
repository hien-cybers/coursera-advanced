...
async function loadOrders() {
  const token = JSON.parse(
    localStorage.getItem("coursera_user_session"),
  )?.token;
  if (!token) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      showToast(
        errorData.message || "Không thể tải dữ liệu đơn hàng.",
        "error",
      );
...
function renderOrders() {
  ...
  document.getElementById("btn-delete-orders") &&
    document.getElementById("btn-delete-orders").classList.add("hidden");
  document.getElementById("btn-delete-orders") &&
    document.getElementById("btn-delete-orders").classList.remove("flex");
  ...
  if (btnDeleteOrders && !document.getElementById("btn-clear-cancelled")) {
    ...
    clearBtn.className =
      "px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 font-bold rounded-lg text-sm transition-colors flex items-center gap-2 ml-2";
  ...
  const filteredOrders = allOrders.filter((order) => {
    const matchesSearch =
      order.user_fullname.toLowerCase().includes(searchTerm) ||
...
function exportOrdersToCSV() {
  ...
  }); // Thêm BOM để Excel không lỗi font
    ...
    async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          button.closest(".action-buttons-wrapper").innerHTML =
            '<span class="text-green-500 font-bold text-xs bg-green-50 px-2 py-1.5 rounded-md border border-green-200"><i class="fa-solid fa-check"></i> Đã duyệt</span>';
    ...
    async () => {
      ...
      try {
        ...
        if (response.ok) {
          // Ẩn lập tức các nút và thay thế bằng text
          button.closest(".action-buttons-wrapper").innerHTML =
            '<span class="text-red-500 font-bold text-xs bg-red-50 px-2 py-1.5 rounded-md border border-red-200"><i class="fa-solid fa-xmark"></i> Đã hủy</span>';
          setTimeout(() => loadOrders(), 1000);
        } else {
          button.disabled = false;
...
async function deleteSelectedOrders() {
  ...
  showConfirmModal(
    `Bạn có chắc chắn muốn xóa ${ids.length} đơn hàng đã chọn? Hành động này không thể hoàn tác.`,
    async () => {
      const token = JSON.parse(
        localStorage.getItem("coursera_user_session"),
      )?.token;
      ...
      for (const id of ids) {
        try {
          const res = await fetch(`admin_api.php/orders/${id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
          });
      ...
      showToast(
        `Đã xóa thành công ${successCount}/${ids.length} đơn hàng.`,
        "success",
      );
      loadOrders();
    },
...
async function clearAllCancelledOrders() {
  showConfirmModal(
    `Bạn có chắc chắn muốn dọn dẹp (xóa vĩnh viễn) TẤT CẢ các đơn hàng đã bị hủy? Hành động này giúp nhẹ máy chủ và không thể hoàn tác.`,
    async () => {
      const token = JSON.parse(
        localStorage.getItem("coursera_user_session"),
      )?.token;
...
function renderUsers() {
  ...
  document.getElementById("btn-delete-users") &&
    document.getElementById("btn-delete-users").classList.add("hidden");
  document.getElementById("btn-delete-users") &&
    document.getElementById("btn-delete-users").classList.remove("flex");
  ...
  allUsers.forEach((user) => {
    ...
    if (user.role === "admin")
      roleBadge = `<span class="px-2 py-1 text-xs font-bold rounded-full bg-blue-100 text-blue-800">Admin</span>`;
    else if (user.role === "teacher")
      roleBadge = `<span class="px-2 py-1 text-xs font-bold rounded-full bg-purple-100 text-purple-800">Teacher</span>`;
...
function exportUsersToCSV() {
  ...
  const blob = new Blob(["\uFEFF" + csvString], {
    type: "text/csv;charset=utf-8;",
  }); // \uFEFF giúp Excel nhận diện font Tiếng Việt
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
...
function openUserModal(userId = null) {
  ...
  if (userId) {
    ...
    document.getElementById("user-password-input").placeholder =
...
async function deleteUser(userId) {
  showConfirmModal(
    `Bạn có chắc chắn muốn xóa người dùng #${userId}? Hành động này không thể hoàn tác.`,
    async () => {
      const token = JSON.parse(
        localStorage.getItem("coursera_user_session"),
      )?.token;
...
async function deleteSelectedUsers() {
  ...
  showConfirmModal(
    `Bạn có chắc chắn muốn xóa ${ids.length} học viên đã chọn? Hành động này không thể hoàn tác.`,
    async () => {
      const token = JSON.parse(
        localStorage.getItem("coursera_user_session"),
      )?.token;
      ...
      for (const id of ids) {
        try {
          const res = await fetch(`admin_api.php/users/${id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
          });
          if (res.ok) successCount++;
      ...
      showToast(
        `Đã xóa thành công ${successCount}/${ids.length} học viên.`,
        "success",
      );
      loadUsers();
    },
...
function resolveImageUrl(rawUrl) {
  ...
  if (uploadsIndex >= 0) {
    relative = normalized.slice(uploadsIndex + 1);
  }

  const basePath = window.location.pathname.replace(/\/[^/]*$/, "");
  if (relative.startsWith("uploads/"))
    return `${basePath ? basePath + "/" : ""}${relative}`;
  if (relative.startsWith("/uploads/")) return `${basePath}${relative}`;
...
function renderAdminCourses() {
  document.getElementById("btn-delete-courses") &&
    document.getElementById("btn-delete-courses").classList.add("hidden");
  document.getElementById("btn-delete-courses") &&
    document.getElementById("btn-delete-courses").classList.remove("flex");
...
async function deleteCourse(courseId) {
  showConfirmModal(
    `Bạn có chắc chắn muốn xóa khóa học '${courseId}'? Hành động này sẽ xóa khóa học khỏi hệ thống.`,
    async () => {
      const token = JSON.parse(
        localStorage.getItem("coursera_user_session"),
      )?.token;
...
async function deleteSelectedCourses() {
  ...
  showConfirmModal(
    `Bạn có chắc chắn muốn xóa ${ids.length} khóa học đã chọn? Hành động này không thể hoàn tác.`,
    async () => {
      const token = JSON.parse(
        localStorage.getItem("coursera_user_session"),
      )?.token;
      ...
      for (const id of ids) {
        try {
          const res = await fetch(`admin_api.php/courses/${id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
          });
          if (res.ok) successCount++;
      ...
      showToast(
        `Đã xóa thành công ${successCount}/${ids.length} khóa học.`,
        "success",
      );
      loadAdminCourses();
    },
...
function openLessonEditModal(courseId, weekId, lessonId) {
  ...
  document.getElementById("edit-lesson-quiz-question").value =
    lesson.quiz_question || "";
  document.getElementById("edit-lesson-quiz-a").value =
    lesson.quiz_option_a || "";
  document.getElementById("edit-lesson-quiz-b").value =
    lesson.quiz_option_b || "";
  document.getElementById("edit-lesson-quiz-correct").value =
...
async function handleLessonSubmit(event) {
  ...
  const body = {
    ...
    quiz_correct_answer: document.getElementById("edit-lesson-quiz-correct")
      .value,
...
async function handleAddWeekSubmit(event) {
  ...
  const body = {
    week_number:
      parseInt(document.getElementById("add-week-number").value) || 1,
...
async function handleEditWeekSubmit(event) {
  ...
  const body = {
    week_number:
      parseInt(document.getElementById("edit-week-number").value) || 1,
...
function renderDiscounts() {
  document.getElementById("selectAllDiscounts") &&
    (document.getElementById("selectAllDiscounts").checked = false);
  document.getElementById("btn-delete-discounts") &&
    document.getElementById("btn-delete-discounts").classList.add("hidden");
  document.getElementById("btn-delete-discounts") &&
    document.getElementById("btn-delete-discounts").classList.remove("flex");
...
async function deleteDiscount(id) {
  showConfirmModal("Bạn có chắc chắn muốn xóa mã giảm giá này?", async () => {
    ...
    try {
      const response = await fetch(`admin_api.php/discounts/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
...
async function deleteSelectedDiscounts() {
  ...
  showConfirmModal(
    `Bạn có chắc chắn muốn xóa ${ids.length} mã giảm giá đã chọn?`,
    async () => {
      const token = JSON.parse(
        localStorage.getItem("coursera_user_session"),
      )?.token;
      ...
      for (const id of ids) {
        try {
          const res = await fetch(`admin_api.php/discounts/${id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
          });
          if (res.ok) successCount++;
      ...
      showToast(
        `Đã xóa thành công ${successCount}/${ids.length} mã giảm giá.`,
        "success",
      );
      loadDiscounts();
    },
